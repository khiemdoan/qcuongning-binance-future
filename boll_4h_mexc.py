...
if __name__ == "__main__":
    ...
    while True:
        ...
        next_kline_start_time = kline_start_time + timedelta(minutes=int(interval[:-1])*60)
        now = datetime.now()
        ...
        syms = binance_sym()
        choice_syms = ['C98USDT', 'LDOUSDT', 'ORDIUSDT', 'DYDXUSDT', 'TIAUSDT']
        ...
        for sym in choice_syms:
            ...
            try:
                ...
                att = previous_close - previous_open

                if att > 0 and previous_open + att * 0.4 < upper_band[-1] < previous_open + att * 0.8:
                    post_tele(f"{time_kline} {sym} meet require {previous_open:.4f} <{upper_band[-1]:.4f}, wait price at {previous_close:.4f}")
                    pick.append(sym)
